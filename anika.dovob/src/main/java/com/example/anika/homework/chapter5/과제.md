...
왜 null일까요?
db에 반영되려면 연관관계 주인(FK를 관리)인 member에서 setTeam으로 설정해야 한다. 
...
flush 전에는 아직 team이 db에 반영되지 않아서 team.members에 member가 없는 상태이다. 
member.setTeam(team)만 호출하고 team.getMembers().add(member)를 호출하지 않았기 size가 0이다. 
flush 후(member, team db 저장 후)에 team을 다시 조회하면 @OneToMany 관계에 의해 member가 team_id FK로 연결되어 있기 때문에 size가 1이 된다. 

```bash
./gradlew test --tests "homework.chapter5.*"
```
