...
5. SQL 로그 비교:
    - 조인 테이블이 생기는 경우는? @JoinColumn이 없을 때
    - UPDATE 쿼리가 나가는 경우는? @JoinColumn이 있을 때

```bash
./gradlew test --tests "homework.chapter6.homework1.*"
```
...
```bash
./gradlew test --tests "homework.chapter6.homework3.*"
```
...
FK가 없는 Locker는 연관 엔티티 Member가 있는지 없는지 알 수 없어서 LAZY 로딩이 제대로 동작하지 않고 Locker 조회 시점에 Member도 함께 조회된다. 
...
1. 숙제 1 (일대다 단방향): Team 1개, Member 2개 저장 시 총 쿼리 몇 개? @JoinColumn 없을 경우 insert 5개 / @JoinColumn 있을 경우 insert 3개, update 2개
2. 다대일 양방향으로 변경해서 같은 테스트 실행 -> insert 3개
